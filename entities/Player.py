...
class Player(BaseEntity):
    ...
    def move(self, dx, dy,obstacles):
        ...
        for obstacle in obstacles:                  
            ...
            if (self.x + self.speed * dx, self.y + self.speed * dy) == (obstacle_x, obstacle_y):         
               return 
        ...
        while self.runTime:     
            for event in pygame.event.get():         
                if event.type == pygame.QUIT:             
                    self.runTime = False
            red_value = int((self.startFrame / self.framerate) * 255)          # Create the color with the changing red component     
